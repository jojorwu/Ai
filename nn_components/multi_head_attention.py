...
class MultiHeadAttention(nn.Module):
    ...
    def _split_heads(self, x: torch.Tensor, num_heads: int) -> torch.Tensor:
        ...
        return x.view(batch_size, seq_len, num_heads, self.d_k).transpose(1, 2)
    ...
    def forward(self, x: torch.Tensor, mask=None, kv_cache=None, layer_idx=None):
        ...
        if kv_cache is not None:
            ...
            # Create attention mask for cached sequence
            mask = torch.triu(torch.ones(seq_len, k_proj.size(2)), diagonal=1).bool().to(x.device)

        k_proj = self._repeat_kv(k_proj, self.num_q_per_kv)
        v_proj = self._repeat_kv(v_proj, self.num_q_per_kv)
