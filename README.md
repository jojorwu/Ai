...
- **Модульная структура**: Все компоненты (`Embedding`, `MultiHeadAttention`, `LayerNormalization` и т.д.) реализованы как отдельные классы в директории `nn_components/`.
- **GELU активация**: В `FeedForward` блоках используется функция активации GELU вместо стандартной ReLU для более гладкой аппроксимации.
