...
- **Оптимизатор Adam**: Для обучения используется продвинутый оптимизатор `Adam` с обрезкой градиентов (gradient clipping).
...
- **Численно стабильная функция потерь**: Для вычисления потерь используется комбинация `log_softmax` и `Negative Log-Likelihood`, что обеспечивает большую численную стабильность по сравнению со стандартным подходом `softmax` + `CrossEntropy`.
...
- `optimizer`: параметры оптимизатора Adam (`learning_rate` здесь выступает как `max_lr`, `beta1`, `beta2`) и порог для обрезки градиентов (`max_norm`).
