...
from src.model.layers.blocks.decoder_block import DecoderBlock, ForwardPassInput
...
class TestDecoderBlock(unittest.TestCase):
    ...
    def test_backward_pass_computes_grads(self):
        ...
        self.assertIsNotNone(
            decoder_block.attention_sublayer.mha.qkv_proj.weight.grad
        )
        self.assertIsNotNone(
        ...
        if decoder_block.use_moe:
            self.assertIsNotNone(
                decoder_block.ff_sublayer.ff_layer.gate.weight.grad
            )
            self.assertTrue(
                any(
                    p.grad is not None for p in
                    decoder_block.ff_sublayer.ff_layer.experts[0].parameters()
                )
            )
        else:
            self.assertIsNotNone(
                decoder_block.ff_sublayer.ff_layer.w1.weight.grad
            )
