...
def _setup_environment(args):
    ...
    if resume_dir:
        ...
        config_path, log_path = os.path.join(resume_dir, 'config.json'), os.path.join(model_dir, 'training.log')
        os.makedirs(model_dir, exist_ok=True)
        setup_logging(log_path)
        logging.info("Resuming training from '%s'. New logs will be in '%s'.", args.resume_from, args.model_name)
    else:
        if os.path.exists(model_dir):
            raise FileExistsError(
                "Model directory '%s' already exists. Use --resume-from or choose a new name.", model_dir
            )
        os.makedirs(model_dir)
        config_path, log_path = 'config.json', os.path.join(model_dir, 'training.log')
        setup_logging(log_path)
...
def _initialize_training_state(model, optimizer, model_dir, resume_dir):
    ...
    if resume_dir:
        ...
        if os.path.exists(weights_path):
            model.load_model(weights_path, model.vocab_size, Config.from_json(os.path.join(resume_dir, 'config.json')), model.tokenizer)
            logging.info("Loaded model weights from %s", weights_path)

    if os.path.exists(checkpoint_path):
        state, _ = load_checkpoint(model, optimizer, checkpoint_path)
        start_epoch, current_step, best_val_loss, epochs_no_improve = state.get('epoch', 0), \
            state.get('current_step', 0), state.get('best_val_loss', float('inf')), state.get('epochs_no_improve', 0)
...
def _run_epoch(epoch, loop_config):
    ...
    phase_epoch = epoch if is_pretrain else epoch - loop_config.config.evolution.pretrain_epochs
    total_phase_epochs = loop_config.config.evolution.pretrain_epochs if is_pretrain else \
    ...
    if is_pretrain:
        avg_loss, epoch_time, current_step_delta = loop_config.trainer.train_pretrain_epoch()
    else:
...
def _handle_epoch_end(val_loss, best_val_loss, epochs_no_improve, loop_config):
    ...
    if val_loss < best_val_loss:
        ...
        loop_config.model.save_weights(os.path.join(loop_config.model_dir, 'best_model.npz'),
...
def _run_training_loop(loop_config):
    ...
    start_epoch, current_step, best_val_loss, epochs_no_improve = _initialize_training_state(
        loop_config.model, loop_config.optimizer, loop_config.model_dir, loop_config.resume_dir)
    total_epochs = loop_config.config.evolution.pretrain_epochs + loop_config.config.evolution.evolution_epochs
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        best_val_loss, epochs_no_improve = _handle_epoch_end(val_loss, best_val_loss,
                                                             epochs_no_improve, loop_config)
        state = {'epoch': epoch + 1, 'current_step': current_step, 'best_val_loss': best_val_loss,
                 'epochs_no_improve': epochs_no_improve}
        save_checkpoint(loop_config.model, loop_config.optimizer, state,
                        loop_config.config.model_dump(), os.path.join(loop_config.model_dir, 'checkpoint.npz'))
        if epochs_no_improve >= loop_config.config.evolution.early_stopping_patience:
...
def main():
    ...
    logging.info("Model initialized with %d trainable parameters.", model.count_parameters())

    trainer = Trainer(model=model, optimizer=optimizer, loss_fn=loss_fn, tokenizer=tokenizer,
                      train_data=train_data, val_data=val_data, config=config)

    loop_config = TrainingLoopConfig(trainer=trainer, config=config, model=model,
                                     optimizer=optimizer, model_dir=model_dir, resume_dir=resume_dir)
