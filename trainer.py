...
class Trainer:
    ...
    def run_validation(self):
        ...
        batch_size = self._config.config.evolution.batch_size
        seq_len = self._config.config.evolution.seq_len
        batch_iterator = get_batches(self._config.val_data, batch_size,
                                     seq_len)
    ...
    def _initialize_pretrain_epoch(self):
        """Initializes variables for a pre-training epoch."""
        ...
        num_batches = len(self._config.train_data) // (evo_config.batch_size *
                                                      evo_config.seq_len)
        training_steps = (num_batches // evo_config.gradient_accumulation_steps *
                          evo_config.pretrain_epochs)
        return batch_iterator, num_batches, training_steps

    def _run_pretrain_step(self, x, y, current_step, training_steps):
        """Runs a single pre-training step."""
        evo_config = self._config.config.evolution
        scheduler_config = self._config.config.scheduler
        ...
        if (current_step + 1) % evo_config.gradient_accumulation_steps == 0:
    ...
    def train_pretrain_epoch(self):
        ...
        batch_iterator, num_batches, training_steps = self._initialize_pretrain_epoch(
        )

        self._config.model.zero_grad()
        for i, (x, y) in enumerate(batch_iterator):
            total_policy_loss += self._run_pretrain_step(
                x, y, i, training_steps)
            if (i + 1) % self._config.config.evolution.gradient_accumulation_steps == 0:
                current_step += 1
